...
QUnit.test("defaultMessage(), empty title is ignored", function (assert) {
	...
	assert.equal(v.defaultMessage($("#username")[0], "required"), "این فیلد اجباری است");
});
...
QUnit.test("previousValue()", function (assert) {
	...
	var e = $("#username"),
		...
		expectedRemote = {
			old: null,
			valid: true,
			message: "لطفا این فیلد را درست کنید"
		},
		expectedRequired = {
			old: null,
			valid: true,
...
QUnit.test("#741: move message processing from formatAndAdd to defaultMessage", function (assert) {
	...
	assert.equal(v.defaultMessage($("#tF22Input")[0], {
			method: "minlength",
			parameters: 5
		}),
...
QUnit.test("", function (assert) {
	...
	assert.deepEqual($(numberInput).rules(), {
		required: true,
		number: true
	});
...
QUnit.test("works on contenteditable fields", function (assert) {
	...
	form.valid();
	assert.hasError($("#contenteditableNumberInvalid"), "لطفا یک شماره معتبر وارد کنید");
	assert.hasError($("#contenteditableRequiredInvalid"), "این فیلد اجباری است");
	assert.hasError($("#contenteditableInput"), "لطفا یک شماره معتبر وارد کنید");
...
QUnit.test("jQuery.validator.format", function (assert) {
	assert.equal(
		jQuery.validator.format("لطفا یک مقدار بین {0} و {1} وارد کنید", 0, 1),
	...
	template = jQuery.validator.format("لطفا یک مقدار بین {0} و {1} وارد کنید");
